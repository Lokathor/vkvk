...
use crate::{strip_number, strip_vendor, vk_dot_xml_parser::TypeEntry};
...
pub fn define_bitmask(
  ...
  if is_alias_for.is_none() {
    if let Some(ty_src) = ty_src {
      assert!(["VkFlags", "VkFlags64"].contains(ty_src));
    }
  }
  ...
  let name = flags.strip_suffix("FlagBits").expect("flag_bits should end with FlagBits");
  //
  ...
  if let Some(alias) = is_alias_for {
    writeln!(f, "/// Khronos: [{alias}](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/{alias}.html) (bitmask)").ok();
    writeln!(f, "pub type {name}FlagBits{num}{ext} = {alias};").ok();
    writeln!(f, "/// Khronos: [{alias}](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/{alias}.html) (bitmask)").ok();
    writeln!(f, "pub type {name}Flags{num}{ext} = {alias};").ok();
  } else {
    writeln!(f, "define_bitmask!(").ok();
    writeln!(f, "  /// Khronos: [{name}FlagBits{num}{ext}](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/{name}FlagBits{num}{ext}.html) (bitmask)").ok();
    writeln!(f, "  {name}FlagBits{num}{ext}").ok();
    writeln!(f, ");").ok();
    writeln!(f, "/// Khronos: [{name}FlagBits{num}{ext}](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/{name}FlagBits{num}{ext}.html) (bitmask)").ok();
    writeln!(f, "pub type {name}Flags{num}{ext} = {name}FlagBits{num}{ext};").ok();
  }
  f
}
