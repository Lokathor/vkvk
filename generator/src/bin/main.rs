...
fn main() {
  ...
  let path = std::path::Path::new("./../src/generated/enumerations.rs");
...
pub fn gather_enumerations(
  ...
  for requirement in all_requirements {
    ...
    } in requirement.required_offset_enums.iter().cloned()
    {
      ...
      match e.offsets.entry(name) {
        Entry::Vacant(ve) => {
          ve.insert(offset);
        }
        Entry::Occupied(oe) => assert_eq!(oe.get(), &offset),
      }
    }
      ...
      requirement.required_value_enums.iter().cloned()
    {
      ...
      let e = output.get_mut(extends).unwrap();
      let mut value = ConstNewtypeValue { name, ty: extends, value, comment };
      if value.name == "VK_SAMPLER_ADDRESS_MODE_MIRROR_CLAMP_TO_EDGE" {
        value.comment = None;
      }
      match e.values.entry(name) {
...
pub struct Enumeration {
  ...
  pub offsets: BTreeMap<StaticStr, ConstNewtypeOffset>,
  pub aliases: BTreeMap<StaticStr, ConstAlias>,
  pub signed: bool,
}
impl Display for Enumeration {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let Enumeration { name, values, aliases, signed, offsets } = self;
...
impl Display for ConstNewtypeOffset {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    let ConstNewtypeOffset { name, ty, ext, offset, is_negative, comment } = self;
    if let Some(comment) = comment {
      writeln!(f, r#"/// {comment}"#)?;
    }
    let value = if *is_negative {
      -extension_enumeration_value(*ext, *offset)
    } else {
      extension_enumeration_value(*ext, *offset)
    };
    writeln!(f, "pub const {name}: {ty} = {ty}({value});")
